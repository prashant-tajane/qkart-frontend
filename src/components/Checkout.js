import { CreditCard, Delete, SettingsRemoteSharp } from "@mui/icons-material";
import {
  Button,
  Divider,
  Grid,
...
const Checkout = () => {
  ...
  const performCheckout = async (token, items, addresses) => {
    const cartValue = getTotalCartValue(items);
    const walletBalance = localStorage.getItem("balance");
    const remainingWalletBalance = walletBalance - cartValue;
    ...
    try {
      ...
      localStorage.setItem("balance", remainingWalletBalance);
      history.push("/thanks");
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Button
              onClick={() => validateRequest(items, addresses)}
