...
export const generateCartItemsFrom = (cartData, productsData) => {
  ...
  cartData.forEach((cartItem) => {
    const productId = cartItem.hasOwnProperty("productId")
      ? cartItem.productId
      : cartItem._id;
    const [obj] = productsData.filter(
...
export const getTotalCartValue = (items = []) => {
  const totalCartValue = items.reduce(
    (accumulator, item) => accumulator + item.cost * item.qty,
    0
  );
...
const Cart = ({ isReadOnly, products, items = [], handleQuantity }) => {
  ...
  if (!items.length) {
    return (
      <Box className="cart empty" backgroundColor="#E9F5E1">
  ...
  const totalCartValue = getTotalCartValue(cartItems);
        ...
        {isReadOnly && (
          ...
          >
            ...
            >
              ...
              <Typography>${totalCartValue}</Typography>
            </Box>
            <Box
              display="flex"
              flexDirection="row"
